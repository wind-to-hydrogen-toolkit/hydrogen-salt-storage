...
def cavern_volume(height, diameter=80, theta=20):
    ...
    ideal cavern volume :math:`V_{ideal}` can therefore be derived by
    ...
    The corrected cavern volume is approximated by applying several correction
    factors as detailed in [#Williams22]_, Eqn. (1).

    .. math::
        V_{cavern} = V_{ideal} * SCF * (1 - IF * INSF * BF)
    """
...
def density_hydrogen_gas(p_operating_min, p_operating_max, t_mid_point):
    ...
    See the CoolProp documentation for hydrogen [#BellCoolProp]_.
